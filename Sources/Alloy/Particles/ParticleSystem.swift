...
class AEParticlesAsset: AEAsset {
    private init() {}
...
class AEParticleSystem: AEGameObject {
    ...
    override func performUpdate(deltaTime: Float) {
        let aliveParticlesPtr = self.aliveCounterBuffer.contents().bindMemory(to: UInt32.self, capacity: 1)
    ...
    private func emitParticles(count: UInt32, commandBuffer: MTLCommandBuffer) {
        ...
        commandEncoder.setBytes(&emitterUniforms, length: MemoryLayout<EmitterUniforms>.stride, index: 1)
        commandEncoder.setBytes(&modelUniforms, length: MemoryLayout<ModelUniforms>.stride, index: 2)
